...
public class FixedAreaAction extends BaseAction<FixedArea> {
	...
	public String pageQuery() {
		...
		Specification<FixedArea> specification = new Specification<FixedArea>() {
			...
			public Predicate toPredicate(Root<FixedArea> root,
				...
				return cb.and(list.toArray(new Predicate[0]));//toArray里的参数是指定转成何种类型的数组
			}
		};
		// 调用业务层，查询数据
	...
	public String findHasAssociationFixedAreaCustomers() {
		...
		System.out.println(collection+"conllection集合");
		ActionContext.getContext().getValueStack().push(collection);
		return SUCCESS;
	}

	// fiexdArea的id用的是模型驱动，客户的id是一个个option的value值，所以这里要用属性驱动，且值为数组
	...
	public String associationCustomersToFixedArea() {
		...
		WebClient.create(
				...
				null);//这里没有指定的对象，我们就用使用null即可
		//这里的customer如果传进来是null，但是这里经过拼接之后就变成了"null",所以在那一边判断要当做字符串判断
		return SUCCESS;
	}

	// 属性驱动，需要快递员的id,定区的id,收派时间的id，其中定区的id通过模型驱动得到的
