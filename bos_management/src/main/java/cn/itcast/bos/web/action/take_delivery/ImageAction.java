...
public class ImageAction extends BaseAction<Object> {
	...
	private File videoFile;//上传的视频文件
	private String videoContentType;//文件类型
	private String videoFileFileName;//文件名称
	...
	public String upload() throws IOException{
		...
		//得到图片的扩展名
		String exName = imgFileFileName.substring(imgFileFileName.lastIndexOf("."));
		...
		result.put("error", 0);
		result.put("url", saveUrl+randomFileName);//上传文件后的相对路径
	...
	public String manage() {
		...
		result.put("current_dir_path", rootPath);//保存图片文件夹的绝对路径
		result.put("current_url", rootUrl);//相对路径
		result.put("total_count", fileList.size());//装载文件信息集合的长度
		result.put("file_list", fileList);//装载文件信息的集合
		ActionContext.getContext().getValueStack().push(result);
