...
<html>
		...
		Date.prototype.format = function(format){ 
			...
			function doDelete(){
				...
				if(rows.length == 0){
					...
					//生成字符串
					var ids = arr.join(",");
					window.location.href = "/talent/delete?talent=photographer&ids="+ids;
				}
			}
			...
			function doUploadImg(){
				...
				if(rows.length == 0){
					// 没选 或 多选 
					$.messager.alert("提示信息","请选择您要上传的摄影师","warning");
				}else if(rows.length != 1){
					// 没选 或 多选 
					...
					var row = rows[0]; 
					// 进行表单回显操作 ，只要使用了它就能将本行的数据加载打表单中去
					$.cookie("photographerId", row.weddingTalentId+","+row.name, {path: '/',expires: 1});
					$.cookie("photographerImg",row.headImg,{path: '/',expires: 1});
					$.cookie("photographerName",row.name,{path: '/',expires: 1});
					location.href="/pages/base/photographerUploadImg";
			...
			function doUploadMedia(){
				...
				if(rows.length == 0){
					...
					$.cookie("photographerImg",row.headImg,{path: '/',expires: 1});
					$.cookie("photographerName",row.name,{path: '/',expires: 1});
					location.href="/pages/base/photographerUploadMedia";
			...
			function checkImgWorks(){
				...
				if(rows.length == 0){
					...
					var row = rows[0]; 
					// 进行表单回显操作 ，只要使用了它就能将本行的数据加载打表单中去
					$.cookie("checkPhotographerId", row.weddingTalentId, {path: '/',expires: 1});
					location.href="/pages/base/photographerImgWorks";
			...
			function checkMediaWorks(){
				...
				if(rows.length == 0){
					...
					// 进行表单回显操作 ，只要使用了它就能将本行的数据加载打表单中去
					$.cookie("checkPhotographerId", row.weddingTalentId, {path: '/',expires: 1});
					location.href="/pages/base/photographerMediaWorks";
			...
			function checkComment(){
				...
				if(rows.length == 0){
					...
					// 进行表单回显操作 ，只要使用了它就能将本行的数据加载打表单中去
					$.cookie("checkPhotographerId", row.weddingTalentId, {path: '/',expires: 1});
					location.href="/pages/base/photographer_comment";
			...
			$(function(){
				...
				$('#grid').datagrid( {
					...
					pagination : true,
					toolbar : toolbar,
					url : "/talent_pageQuery?name="+encodeURI("摄影师")+"&hotelId="+$.cookie("yonghuming"),
	...
	<body class="easyui-layout" style="visibility:hidden;">
		...
		<div class="easyui-window" title="摄影师添加" id="addWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
			...
			<div region="center" style="overflow:auto;padding:5px;" border="false">
				<form id="addForm" action="/talent/add?talent=photographer" method="post" enctype="multipart/form-data">
					<table class="table-edit" width="80%" height="80%" align="center">
						...
						<tr>
							...
							<td>
								...
								<input type="hidden" value="摄影师"  name="occupation"/>
							</td>
						</tr>
		...
		<div class="easyui-window" title="摄影师修改" id="eidtorWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
			<div region="north" style="height:31px;overflow:hidden;" split="false" border="false">
				<!--<div class="datagrid-toolbar">-->
			...
			<div region="center" style="overflow:auto;padding:5px;" border="false">
					<form id="updateForm" action="/talent/update?talent=photographer" method="post" enctype="multipart/form-data">
					<table class="table-edit" width="80%" height="80%" align="center">
						<tr class="title">
							<td colspan="2">人才信息</td>
