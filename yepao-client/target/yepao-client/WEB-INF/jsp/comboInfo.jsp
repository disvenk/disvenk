	...
	<head>
		...
		Date.prototype.format = function(format){ 
			...
			function checkComment(){
				...
				if(rows.length == 0){
					// 没选 或 多选 
					$.messager.alert("提示信息","请选择您要查看的菜品套餐","warning");
				}else if(rows.length != 1){
					// 没选 或 多选 
					...
					$.cookie("checkComboId", row.id, {path: '/',expires: 1});
					location.href="/pages/base/combo_comment";
			...
			$(function(){
				...
				$('#grid').datagrid( {
					...
					pagination : true,
					toolbar : toolbar,
					url : "/combo_pageQuery?hotelId="+$.cookie("yonghuming"),
			...
			function doAssociations(){
				...
				if(rows.length == 1){
					...
					// 发起Ajax请求 
					$.post("/dishStyle_pageQuery", {hotelId:'${cookie.yonghuming.value}'},function(data){
							// 清空列表
